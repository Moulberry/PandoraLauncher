...
fn build_http_clients(
    ...
    let mut ollama_builder = reqwest::ClientBuilder::new()
        .connect_timeout(Duration::from_secs(15))
    ...
    if let Some(proxy_url) = &proxy_url {
        if let Ok(proxy) = reqwest::Proxy::all(proxy_url) {
            ...
            redirecting_builder = redirecting_builder.proxy(proxy.clone());
            ollama_builder = ollama_builder.proxy(proxy);
            log::info!(
                "Proxy configured: {}://{}:{}",
    ...
    let ollama_http_client = ollama_builder.build().expect("Failed to build Ollama HTTP client");

    (http_client, redirecting_http_client, ollama_http_client)
}
...
pub fn start(
    ...
    let (http_client, redirecting_http_client, ollama_http_client) =
...
pub struct BackendState {
    ...
    pub ollama_http_client: reqwest::Client,
