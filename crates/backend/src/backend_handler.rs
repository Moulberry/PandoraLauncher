            ...
            MessageToBackend::RequestMetadata { request, force_reload } => {
                ...
                tokio::task::spawn(async move {
                    let (result, keep_alive_handle) = match request {
                        ...
                        bridge::meta::MetadataRequest::LegacyFabricLoaderManifest => {
                            let (result, handle) = meta.fetch_with_keepalive(&LegacyFabricLoaderManifestMetadataItem, force_reload).await;
                            (result.map(MetadataResult::LegacyFabricLoaderManifest), handle)
                        },
