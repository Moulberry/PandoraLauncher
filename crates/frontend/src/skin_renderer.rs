        ...
        for px in min_x..=max_x {
            ...
            if w0 >= 0.0 && w1 >= 0.0 && w2 >= 0.0 {
                ...
                if z > zbuf[idx] {
                    ...
                    let tx = ((u * uv_scale_x).floor() as u32).min(skin_w - 1);
                    let ty = ((v * uv_scale_y).floor() as u32).min(skin_h - 1);
...
impl ProjectedQuad {
    fn rasterize(&self, skin: &image::DynamicImage, output: &mut RgbaImage, zbuf: &mut [f64]) {
        let (uv_scale_x, uv_scale_y) = if self.part_type == BodyPartType::Cape {
            (skin.width() as f64 / 64.0, skin.height() as f64 / 32.0)
        } else {
            (1.0, 1.0)
        };
...
fn normalize_cape_texture(image: DynamicImage) -> DynamicImage {
    let source = image.to_rgba8();
    let (src_w, src_h) = (source.width(), source.height());

    // OptiFine capes are 46x22, or an integer scale of that. The left 22x17
    // region is the cape; the right 24x22 region is elytra data. Preserve that
    // layout by pasting/downsampling the OptiFine image into a normal 64x32 cape
    // canvas instead of sampling the elytra half as cape texture.
    if src_w % 46 == 0 {
        let ratio = src_w / 46;
        if ratio > 0 && src_h == 22 * ratio {
            let mut canvas = RgbaImage::new(64 * ratio, 32 * ratio);
            for y in 0..src_h {
                for x in 0..src_w {
...
pub fn render_skin_3d(
    ...
    let cape = cape_png_bytes
        .map(|cape| {
            image::load_from_memory_with_format(cape, ImageFormat::Png)
                .ok()
                .map(normalize_cape_texture)
        })
        .flatten();
